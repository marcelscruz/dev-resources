...
const extractResourcesFromFile = (filePath) => {
  try {
      ...
      if (!inString) {
        if (char === '{') {
          ...
          if (braceCount === 0) {
            ...
            while (i + 1 < arrayContent.length && (arrayContent[i + 1] === ',' || /\s/.test(arrayContent[i + 1]))) {
              i++;
            }
          }
    ...
    const parsedResources = resources.map(objStr => {
      try {
        // Extract name
        ...
        // Extract url
        const urlMatch = objStr.match(/url:\s*['"](.+?)(?<!\\)['"]/) || objStr.match(/url:\s*['"](.+?)['"]/) || ['', ''];
